...
def se_2D_v0_RP():
    ...
    for idx2 in range(pe_step_nr):
        ...
        if False:  # set to True/(False) if you want to plot the x gradient waveform
            ...
            plt.show()
            time.sleep(0.5)
        grad_idx = exp.add_grad(grad_pe_corr * scale_G_pe_sweep, grad_pe_corr, np.zeros(np.size(grad_pe_corr)))
        grad_idx = exp.add_grad(grad_fe_corr, np.zeros(np.size(grad_fe_corr)), np.zeros(np.size(grad_fe_corr)))
